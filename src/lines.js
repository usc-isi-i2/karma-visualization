       ...
       /* bigrect = svg.append("rect")
        ...
        var tickforcluster = function(e)
        {
            ...
            sticks.attr("x1", function(d){return d.x;})
                  ...
                  .attr("y1", function(d){return d.y + sizescale(d.size);})
                  .attr("y2", function(d){return d.y - sizescale(d.size);});
        }
        ...
        var collide = function(alpha)
        {
            return function(node1)
            {
                datacircle.forEach(function(node2){
                        ...
                        var shortest = sizescale(node1.size)/8 + sizescale(node2.size)/8 + padding;
                       // alert(shortest);
                        if(dis < shortest)
                        {
        ...
        d3.json(filename, function(err, data){
                ...
                sticks = sticks.data(datacircle)
                               .enter()
                               .append("line")
                               .style("stroke", function(d){ if(havecolor)
                                            return colorscale(d.color);
                                         else
                                         {
                                            d.color = colorscale(c);
